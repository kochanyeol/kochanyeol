...
def todo_info(request, id):
    # url에서 넘어온 id로 Todo 객체를 가져오고 없으면 404 반환
...
def todo_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():  # 입력값이 유효한지 검사
            ...
            return redirect('todo_info', id=todo.pk)  # 생성된 Todo 상세 페이지로 이동
    else:
        # GET 요청 시 빈 폼을 렌더링
        form = TodoForm()
...
def todo_update(request, id):
    # id와 로그인한 유저가 일치하는 Todo 가져오기 (없으면 404 반환)
    todo = get_object_or_404(Todo, pk=id, user=request.user)
    if request.method == 'POST':
        ...
        if form.is_valid():  # 입력값이 유효한지 검사
            ...
            return redirect('todo_info', id=todo.pk)  # 수정된 Todo 상세 페이지로 이동
    else:
